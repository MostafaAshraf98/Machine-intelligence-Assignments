...
from value_iteration import ValueIterationAgent, AsynchronousValueIterationAgent
...
def print_results(env: GridEnv, agent: Agent[Point, Direction]):
    ...
    if isinstance(agent, ValueIterationAgent):
        ...
        print()
    elif isinstance(agent, SARSALearningAgent) or  isinstance(agent, QLearningAgent) or  isinstance(agent, ApproximateQLearningAgent):
        print("Q-Values:")
...
def train_value_iteration(args: argparse.Namespace):
    ...
    elif agent_type == "async_value_iteration":
        print("Training an Asynchronous Value Iteration Agent...")
        agent = AsynchronousValueIterationAgent(mdp, args.discount)
    else:
        print(f"Requested Agent '{agent_type}' is invalid")
        exit(-1)
        
    # Apply value iteration for the given number of iterations
    agent.train(int(args.iterations))
...
def train_q_learning(args: argparse.Namespace):
    ...
    # Create the requested agent (tabular or approximate)
    agent_type = args.agent
...
def main(args: argparse.Namespace):
    ...
    if agent_type.endswith("value_iteration"):
...
if __name__ == "__main__":
    ...
    parser.add_argument("agent", type=str, choices=["value_iteration", "async_value_iteration", "sarsa", "q_learning", "q_learning_approx"])
    ...
    parser.add_argument("--verbosity", "-v", type=int, default=0, help="How often to display the training results (For SARSA & Q-Learning Only) (0 will display at the end only)")
