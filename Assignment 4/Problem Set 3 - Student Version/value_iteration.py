...
class ValueIterationAgent(Agent[S, A]):
    ...
    def compute_bellman(self, state: S) -> float:
        ...
        for action in self.mdp.get_actions(state):
            # Variable to keep the sum/final result
            result = 0.0
    ...
    def train(self, iterations: int = 1):
        ...
        # Temp variable to hold the new utility
        temp: Dict[str, float]
        temp = {str(state): 0 for state in self.mdp.get_states()}
        ...
        for _ in range(iterations):
            ...
            # store the new utilities in the agent at the end of each iteration
    ...
    def train(self, iterations: int = 10):
       ...
       # Starting from the current utilities stored in the agent
        ...
        for i in range(iterations):
            # Because it is cyclic we will use the remainder
            stateNumber = i % len(self.mdp.get_states())
            # Compute the utility for this state
            self.utilities[str(self.mdp.get_states()[stateNumber])] = self.compute_bellman(
                self.mdp.get_states()[stateNumber])
