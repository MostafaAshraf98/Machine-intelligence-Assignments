...
# Train an async value iteration agent on grid MDP 
def run_async_value_iteration(env: GridEnv, discount_factor: float, iterations: int, noise: float = None) -> Tuple[UtilityMap, Policy]:
    if noise is not None: # If noise is not none, we override the default value supplied with the environment
        env.mdp.noise = noise
    # Create and train the Value Iteration agent
    cls = load_function("value_iteration.AsynchronousValueIterationAgent")
    agent = cls(env.mdp, discount_factor)
    agent.train(iterations)
    # Reset the environment since we need to do that before requesting actions from the agent
    env.reset()
